...
expression
	...
	| {PrefixExpression pe = new PrefixExpression();} (
		pp = PlusPlus {pe.addOperation($pp.text);}
		| mm = MinusMinus {pe.addOperation($mm.text);}
		| s = Sizeof {pe.addOperation($s.text);}
	)* (
		// Operationerand of the prefix expression
		...
		| u = unaryOperator c1 = castExpression {
            pe.setUnaryOperation($u.unaryOperationRet);
	...
	| e1 = expression a = assignmentOperator e2 = expression {$e1.expressionRet.notFirst(); $e2.expressionRet.notFirst();
		} {$expressionRet = new BinaryExpression($e1.expressionRet, $e2.expressionRet, $a.assignmentOperationRet); $expressionRet.setLine($a.assignmentOperationRet.getLine());
		};
...
unaryOperator
	returns[UnaryOperator unaryOperationRet]:
	t = And {$unaryOperationRet = new UnaryOperator($t.text);}
	| t = Star {$unaryOperationRet = new UnaryOperator($t.text);}
	| t = Plus {$unaryOperationRet = new UnaryOperator($t.text);}
	| t = Minus {$unaryOperationRet = new UnaryOperator($t.text);}
	| t = Tilde {$unaryOperationRet = new UnaryOperator($t.text);}
	| t = Not {$unaryOperationRet = new UnaryOperator($t.text);};
...
assignmentOperator
	returns[AssignmentOperation assignmentOperationRet]:
	opToken = Assign { $assignmentOperationRet = new AssignmentOperation($opToken.text); $assignmentOperationRet.setLine($opToken.line); 
		}
	| opToken = StarAssign { $assignmentOperationRet = new AssignmentOperation($opToken.text); $assignmentOperationRet.setLine($opToken.line); 
		}
	| opToken = DivAssign { $assignmentOperationRet = new AssignmentOperation($opToken.text); $assignmentOperationRet.setLine($opToken.line); 
		}
	| opToken = ModAssign { $assignmentOperationRet = new AssignmentOperation($opToken.text); $assignmentOperationRet.setLine($opToken.line); 
		}
	| opToken = PlusAssign { $assignmentOperationRet = new AssignmentOperation($opToken.text); $assignmentOperationRet.setLine($opToken.line); 
		}
	| opToken = MinusAssign { $assignmentOperationRet = new AssignmentOperation($opToken.text); $assignmentOperationRet.setLine($opToken.line); 
		}
	| opToken = LeftShiftAssign { $assignmentOperationRet = new AssignmentOperation($opToken.text); $assignmentOperationRet.setLine($opToken.line); 
		}
	| opToken = RightShiftAssign { $assignmentOperationRet = new AssignmentOperation($opToken.text); $assignmentOperationRet.setLine($opToken.line); 
		}
	| opToken = AndAssign { $assignmentOperationRet = new AssignmentOperation($opToken.text); $assignmentOperationRet.setLine($opToken.line); 
		}
	| opToken = XorAssign { $assignmentOperationRet = new AssignmentOperation($opToken.text); $assignmentOperationRet.setLine($opToken.line); 
		}
	| opToken = OrAssign { $assignmentOperationRet = new AssignmentOperation($opToken.text); $assignmentOperationRet.setLine($opToken.line); 
		};
...
directDeclarator
	...
	{ $directDecRet = new DirectDec(); } identifierToken = Identifier { $directDecRet.setIdentifier($identifierToken.text); $directDecRet.setLine($identifierToken.line); 
		}
	| { $directDecRet = new DirectDec(); } leftParenToken = LeftParen nestedDeclarator = declarator
		RightParen { $directDecRet.setDeclarator($nestedDeclarator.declaratorRet); $directDecRet.setLine($leftParenToken.line); 
		}
	| { $directDecRet = new DirectDec(); } // Array declarator
	arrayBaseDeclarator = directDeclarator { $directDecRet.setDirectDec($arrayBaseDeclarator.directDecRet); 
		} LeftBracket (
		arraySizeExpr = expression { $directDecRet.setExpression($arraySizeExpr.expressionRet); }
	)? RightBracket
	| { $directDecRet = new DirectDec(); } // Function declarator
	funcBaseDeclarator = directDeclarator { $directDecRet.setDirectDec($funcBaseDeclarator.directDecRet); 
		...
		| (
			identList = identifierList { $directDecRet.setIdentifierList($identList.identifierListRet); 
				}
		)? // K&R style
	) RightParen;
...
pointer
	...
	// One or more '*' optionally followed by 'const'

// Rule for a comma-separated list of parameter declarations.
parameterList
	returns[ParameterList parameterListRet]:
	firstParameter = parameterDeclaration { $parameterListRet = new ParameterList($firstParameter.parameterDecRet); 
		} (
		Comma nextParameter = parameterDeclaration { $parameterListRet.addParameterDec($nextParameter.parameterDecRet); 
			}
	)*;
...
parameterDeclaration
	...
	paramTypeSpecs = declarationSpecifiers { $parameterDecRet = new ParameterDec($paramTypeSpecs.declarationSpecifiersRet); 
		} (
...
typeName
	...
	specQuals = specifierQualifierList { $typeNameRet = new TypeName($specQuals.specifierQualifierListRet); 
		} (
...
abstractDeclarator
	...
	{ $abstractDecRet = new AbstractDec(); } pointerNode = pointer { $abstractDecRet.setPointer($pointerNode.pointerRet); 
		}
...
directAbstractDeclarator
	...
	LeftBracket (
		arraySizeExpr = expression { $directAbsDecRet.setExpression($arraySizeExpr.expressionRet); }
	)? RightBracket
	| { $directAbsDecRet = new DirectAbsDec(); }
	// Parenthesized abstract declarator or abstract function type: ( abstract_declarator | parameter_list? )
	LeftParen (
	...
	| { $directAbsDecRet = new DirectAbsDec(); }
	// Recursive abstract array declarator: direct_abstract_declarator [ expression? ]
	baseDirectAbstractDeclarator_Array = directAbstractDeclarator { $directAbsDecRet.setDirectAbsDec($baseDirectAbstractDeclarator_Array.directAbsDecRet); 
		} LeftBracket (
		recursiveArraySizeExpr = expression { $directAbsDecRet.setExpression($recursiveArraySizeExpr.expressionRet); 
			}
	)? RightBracket
	| { $directAbsDecRet = new DirectAbsDec(); }
	// Recursive abstract function declarator: direct_abstract_declarator ( parameter_list? )
	baseDirectAbstractDeclarator_Func = directAbstractDeclarator { $directAbsDecRet.setDirectAbsDec($baseDirectAbstractDeclarator_Func.directAbsDecRet); 
		} LeftParen (
		recursiveParamList = parameterList { $directAbsDecRet.setParameterList($recursiveParamList.parameterListRet); 
			}
	)? RightParen;
...
initializer
	...
	{ $initializerRet = new Initializer(); } initExpression = expression { $initializerRet.setExpression($initExpression.expressionRet); 
		}
	| { $initializerRet = new Initializer(); } LeftBrace initItemsList = initializerList { $initializerRet.setInitializerList($initItemsList.initializerListRet); 
		} Comma? RightBrace;
...
designator
	...
	{ $designatorRet = new Designator(); } LeftBracket indexValueExpression = expression
		RightBracket { $designationRet.setExpression($indexValueExpression.expressionRet); }
	| Dot memberNameIdentifier = Identifier { $designationRet.setLine($memberNameIdentifier.line); }
		; // Assuming Designator AST node can store line info for dot member
...
blockItem
	...
	{ $blockItemRet = new BlockItem(); } statementNode = statement { $blockItemRet.setStatement($statementNode.statementRet); 
		}
	| { $blockItemRet = new BlockItem(); } declarationNode = declaration { $blockItemRet.setDeclaration($declarationNode.declarationRet); 
		};
...
iterationStatement
	...
	whileToken = While LeftParen loopConditionExpr = expression RightParen loopBodyStatement =
		statement {
	...
	| Do doLoopBodyStatement = statement doWhileToken = While LeftParen doLoopConditionExpr =
		expression RightParen Semi {
        $iterStatementRet = new IterStatement();
	...
	| forToken = For LeftParen forLoopConditions = forCondition RightParen forLoopBodyStatement =
		statement {
        $iterStatementRet = new IterStatement();
...
forCondition
	...
	{ $forConditionRet = new ForCondition(); } (
		...
		// Iteration part
		iterationExpressionNode = forExpression { $forConditionRet.setForExpression1($iterationExpressionNode.forExpressionRet); 
			} // AST uses ForExpression1 for iteration too
	)?;
// Rule for the declaration part within a for-loop's initialization (e.g., for(int i=0; ...)).
forDeclaration
	returns[ForDec forDecRet]:
	declarationSpecs = declarationSpecifiers { $forDecRet = new ForDec($declarationSpecs.declarationSpecifiersRet); 
		} (
		initDeclarators = initDeclaratorList { $forDecRet.setInitDecList($initDeclarators.initDeclaratorListRet); 
			}
	)?;
...
forExpression
	...
	initialExpression = expression { $forExpressionRet = new ForExpression($initialExpression.expressionRet); 
		} (
		Comma additionalExpr = expression { $forExpressionRet.addExpression($additionalExpr.expressionRet); 
			}
	)*;
