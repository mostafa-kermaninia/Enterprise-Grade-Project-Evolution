    ...
    |  e4=expr op3 = ('+' | '-') e5=expr
        {
            ...
            $exprRet.setLine($op3.line);
        }
    |e03=expr op02=('==' | '>=') e04=expr
             {
                 BinaryOperator  op = ($op02.text.equals("==")) ? BinaryOperator.EQUALS :  BinaryOperator.GTE;
                 $exprRet = new BinaryExpr($e03.exprRet, op, $e04.exprRet);
                 $exprRet.setLine($op02.line);
             }
    |e01=expr op01=('&&' | '||') e02=expr
                 {
                     BinaryOperator  op = ($op01.text.equals("&&")) ? BinaryOperator.AND :  BinaryOperator.OR;
                     $exprRet = new BinaryExpr($e01.exprRet, op, $e02.exprRet);
                     $exprRet.setLine($op01.line);
                 }
...
primary_expr returns [Expr primary_expr_ret]:
    ...
    | c=CHAR_VAL   { $primary_expr_ret = new CharVal($c.text); $primary_expr_ret.setLine($c.line);}
    ;
...
// 1- General structure
MAIN : 'main';
TYPE : 'int' | 'string' | 'double' | 'bool' | 'char';
IF : 'if';
ELSE : 'else';
RETURN : 'return';
